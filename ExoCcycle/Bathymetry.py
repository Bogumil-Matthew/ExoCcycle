...
class BathyMeasured():
    ...
    def readTopo(self, data_dir, new_resolution=1, flipLatitude=False, verbose=True):
        ...
        flipLatitude : boolean, optional
            ...
            be done for some models where input topographies start at  
        verbose : bool, optional
...
class BathyRecon():
    ...
    def __init__(self, directories):
        ...
        # FIXME - Check that there are no issues for pip instalation of ExoCcycle  
        pathScript      = "/".join(os.path.dirname(os.path.realpath(__file__)).split('/')[:-1])
        ...
        # Set output for data - Note that a
        self.data_dir = os.getcwd()+"/bathymetries";
    ...
    def run(self, startMa=80, endMa=0, deltaMyr=5, resolution=1,
            fieldThresholds={},
            maxBasinCnt=1e5, skipfindBasins=False, skipVOCC_BasinStats=False, verbose=True,
            writeReadmeandPlotsOpts={}):
        """
    ...
    def saveBathymetry(self, reconAge, subName="", verbose=True):
        ...
        """
        # Make directory for storing bathymetry model(s)
        utils.create_file_structure([self.data_dir,
                                     self.data_dir+"/EarthRecon",
                                     self.data_dir+"/EarthRecon3Basins",
                                     self.data_dir+"/EarthRecon3_4Basins"],
                                     root = True,
                                     verbose=verbose)     
        
        # Set netCDF4 filename
        BathyPath = "{0}/{1}/{1}_{2:0.1f}deg_{3:0.0f}Ma{4}.nc".format(self.data_dir,  self.model, self.resolution, reconAge, subName);
